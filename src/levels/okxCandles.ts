...
interface OkxHistoryCandlesResponse {
  code?: string;
  msg?: string;
  data?: string[][];
}

/** Fetches `totalNeeded` candles ending at (just before) `endTs`, for backtesting
 * against a specific historical window. Uses /market/history-candles, which OKX
 * keeps for older data than /market/candles serves. Calibration-only tool — not
 * used by the production pipeline. */
export async function fetchOkxHistoryCandlesUntil(
  instId: string,
  bar: string,
  endTs: number,
  totalNeeded: number,
  ...
  while (collected.length < totalNeeded) {
    ...
    const json = (await res.json()) as OkxHistoryCandlesResponse;
    if (json.code !== '0' || !json.data || json.data.length === 0) break;
